...
export function ModelGenerationProgress({ phase }: { phase: AiModelPhase }) {
    ...
    const detail = anchoring
        ? "先生成一张母版锁定人物身份，后续图片都以它为参考图。"
        : finished
          ? `已派生 ${phase.done}/${phase.total} 张。`
          : `已派生 ${phase.done}/${phase.total} 张，均以母版为参考图。`;
    return (
        <div className="flex items-center gap-2.5 rounded-md border border-border bg-surface px-3 py-2.5" aria-live="polite">
            ...
            <div className="min-w-0">
                <div className="text-[var(--fs-label)] font-medium text-foreground">{title}</div>
                <div className="mt-0.5 text-[var(--fs-micro)] leading-4 text-foreground/58">{detail}</div>
            </div>
        </div>
    );
}
