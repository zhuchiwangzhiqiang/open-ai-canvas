...
import { isEmptyModelAttributes, type ModelAttributes, type ModelPreset } from "@/lib/design/model-attributes";
    ...
    return (
        <div className="mb-2.5">
            <div className="flex items-center gap-2">
                <span className="grid size-4 shrink-0 place-items-center rounded-full border border-border text-[var(--fs-micro)] text-foreground/52">{index}</span>
                <span className="text-[var(--fs-label)] font-semibold text-foreground">{title}</span>
                {hint ? <span className="text-[var(--fs-micro)] text-foreground/45">{hint}</span> : null}
            </div>
        </div>
    );
}
...
export default function AiModelPage() {
    ...
    return (
        <WorkspacePage>
            ...
            <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,400px)_minmax(0,1fr)]">
                <section className="space-y-5 rounded-lg border border-border bg-surface p-4" aria-label="AI 模特生成设置">
                    <div>
                        ...
                        />
                        {!referenceSupported ? (
                            <p className="mt-2 text-[var(--fs-micro)] leading-4 text-foreground/52">当前模型不支持参考图，这批图之间无法锁定为同一位模特。</p>
                        ) : null}
                    </div>
                    ...
                    <label className="flex items-center justify-between gap-3 text-[var(--fs-label)] text-foreground/72">
                        <span>派生时自动变化姿势</span>
                        <Switch size="small" checked={variation} disabled={busy} onChange={setVariation} />
                    </label>

                    <div className="space-y-2">
                        <p className="text-[var(--fs-micro)] leading-4 text-foreground/52">生成的模特会进入历史记录，后续可在「AI 模特图」等能力中复用。</p>
                        ...
                        {busy ? <Button block onClick={cancel}>取消生成</Button> : null}
                        {portraitCost !== null && portraitCost !== undefined ? (
                            <p className="text-center text-[var(--fs-micro)] text-foreground/52">
                                预计消耗 {portraitCost.toLocaleString("zh-CN", { maximumFractionDigits: 3 })} 积分
                            </p>
                        ) : null}
                    </div>
                </section>

                <section className="min-w-0" aria-label="AI 模特结果">
                    <div className="flex items-center gap-1 border-b border-border" role="tablist" aria-label="AI 模特结果视图">
                        {(["result", "history"] as const).map((key) => (
                            <button
                                ...
                                className={cn(
                                    "-mb-px h-9 border-b-2 px-3 text-[var(--fs-label)] font-medium transition-colors motion-reduce:transition-none",
                    ...
                    <div className="mt-3">
                        {tab === "result" ? (
                            <ModelResultPanel result={result} phase={phase} error={error} busy={busy} count={PORTRAIT_COUNT} onRetry={() => void generate()} />
                        ) : (
